...
const updateStudentGradeByCity = (students, city, newGrades) => {
  const updated = students.filter((student) => student.location === city).map((student) => {
    const temp = newGrades.find((grade) => grade.studentId === student.id);
    return { ...student, grade: temp ? temp.grade : 'N/A' };
  });
  return updated;
};
